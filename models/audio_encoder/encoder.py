...
from models.modules import ResnetBlock, Res_CNR_Stack, Downsample

class AudioEncoder(nn.Module):
    def __init__(self, in_dim, num_hiddens, num_residual_layers, num_residual_hiddens):
        super(AudioEncoder, self).__init__()
        self._num_hiddens = num_hiddens   ###256
        self._num_residual_layers = num_residual_layers   ###2
        self._num_residual_hiddens = num_residual_hiddens   ###256
        
        self.project = ResnetBlock(in_channels = in_dim, out_channels = self._num_hiddens // 4)

        self._enc_1 = Res_CNR_Stack(self._num_hiddens // 4, self._num_residual_layers, leaky=True)
        self._down_1 = nn.Sequential(ResnetBlock(in_channels = self._num_hiddens // 4, out_channels = self._num_hiddens // 2),
                                     Downsample(self._num_hiddens // 2, True))
        self._enc_2 = Res_CNR_Stack(self._num_hiddens // 2, self._num_residual_layers, leaky=True)
        self._down_2 = nn.Sequential(ResnetBlock(in_channels = self._num_hiddens // 2, out_channels = self._num_hiddens),
                                     Downsample(self._num_hiddens, True))
